...
app.layout = html.Div([
                ...
                html.Label('Seq direction'),
                dcc.Input(id='input-5-submit',
                          type='text',
                          placeholder='Complement, Reverse Complement, Both'),
                html.Br(),
                ...
                dcc.Input(id='input-12-submit',
                          type='number',
                          placeholder='TGG bp from stop'),
                html.Br(),

                html.Label('ATG choice: '),
                dcc.Dropdown(id='input-13-submit',
                    options=[
...
def update_output(clicked, species, geneName, isoForm, searchSeq,
                  typeSeq, lenSes, numTGG, numStop,
    ...
    if clicked:
        ...
        rC_parameters = parameters_sesRNA(typeSeq, (isoForm-1), lenSes, numTGG,
                                          numStop, atgChoice,
                                          minGC, maxGC,
                                          nearCenter, fromStop)
        ...
        all_sesRNAs, all_sequenceMetrics, all_sesRNA_objs = \
            generate_all_sesRNAs(rC_exon_records, C_exon_records,
                                targetSeq, rC_parameters)

        final_sequenceMetrics = \
            generate_RNApred(all_sesRNAs, all_sequenceMetrics, geneName)

        return generate_table(final_sequenceMetrics, max_rows=10)
...
def display_chosenSeq(clicked, species, geneName, isoForm, searchSeq,
                      typeSeq, lenSes, numTGG, numStop,
    ...
    if clicked:
        ...
        parameters = parameters_sesRNA(typeSeq, (isoForm-1), lenSes, numTGG,
                                          numStop, atgChoice,
                                          minGC, maxGC,
                                          nearCenter, fromStop)
...
@app.callback(Output("outputRNA-strSeq", "value"),
                ...
                [State('input-1-submit', 'value'),
                 ...
                 State('input-13-submit', 'value'),
                 State('input-14-submit', 'value'),
                 State('input-15-submit', 'value')],
                 prevent_initial_call=True,
)
def display_chosenRNA(clicked, species, geneName, isoForm, searchSeq,
                      typeSeq, lenSes, numTGG, numStop, minGC,
                      maxGC, nearCenter, fromStop, atgChoice,
                      seqNum, numTGG_convert):
    if clicked:
        filename = "sesRNA.txt"
        # Alternatively:
        # filename = f"{uuid.uuid1()}.txt"
...
def create_download_file(clicked, species, geneName, isoForm, searchSeq,
                         typeSeq, lenSes, numTGG, numStop,
